...
import '../widgets/enhanced_autocomplete_field.dart';
...
class _CustomerBookRidePageState extends State<CustomerBookRidePage> {
  ...
  lat.LatLng? _currentLocation;
  lat.LatLng? _pickupLocation;
  lat.LatLng? _destination;
  ...
  Future<void> _initializeLocation() async {
    ...
    final granted = await OpenStreetMapSearchService.requestPermission();
    if (!granted) {
      setState(() {
        _isLoading = false;
      });
    ...
    final isEnabled = await OpenStreetMapSearchService.isLocationServiceEnabled();
    if (!isEnabled) {
      setState(() {
        _isLoading = false;
      });
    ...
    final loc = await OpenStreetMapSearchService.getCurrentLocation();
    if (loc != null) {
      setState(() {
        _currentLocation = loc.toLatLng();
        _pickupLocation = loc.toLatLng();
        _pickupController.text = 'Current Location';
        _pickupText = 'Current Location';
        _isLoading = false;
      });
      // Map will automatically center when _currentLocation changes
    } else {
  ...
  void _moveToLocation(lat.LatLng location) async {
    // For flutter_map, we just update the state and the map will re-render
    setState(() {
  ...
  void _updateEstimatedPrice() {
    if (_pickupLocation != null && _destination != null) {
      ...
      // Use dynamic pricing service
      final pricing = DynamicPricingService.calculateDynamicFare(
        vehicleType: _selectedVehicle,
        distanceKm: distance,
        durationMin: (distance / 30 * 60), // Estimate duration based on 30 km/h avg speed
      );
      
      setState(() {
        _estimatedPrice = pricing['total'];
      });
    }
  }
  ...
  Widget _buildMap() {
    ...
    // Add driver markers
    allMarkers.addAll(_driverMarkers.map((marker) => marker));
    ...
    return SimpleMap(
      center: _currentLocation ?? lat.LatLng(37.7749, -122.4194),
      zoom: 15.0,
      markers: allMarkers,
      onTap: (dest) {
        setState(() {
  ...
  Future<void> _requestRide() async {
    ...
    try {
      ...
      final pickupLocationModel = LocationModel(
        latitude: latPickup.latitude,
        longitude: latPickup.longitude,
      );
      final destinationLocationModel = LocationModel(
        latitude: latDestination.latitude,
        longitude: latDestination.longitude,
      );
      ...
      final rideId = await RideBookingService.createRideRequest(
        ...
        destinationLocation: destinationLocationModel,
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return Padding(
          ...
          child: EnhancedAutocompleteField(
                  ...
                  child: SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final fare = DynamicPricingService.calculateDynamicFare(
                          vehicleType: vehicle,
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      final location = await OpenStreetMapSearchService.getCurrentLocation();
      if (location != null && mounted) {
        setState(() {
          _currentLocation = location.toLatLng();
        ...
        // Map will automatically update when _currentLocation changes
      } else {
        setState(() {
          _isLoading = false;
        });
