// lib/pages/customer_book_ride_clean.dart
import 'dart:async';
...
// Simple state class with null safety
class RideState {
  ...
  RideState({
    ...
    this.vehicleTypes = const ['Bike', 'Scooty', 'Standard', 'Comfort', 'Premium', 'XL'],
  });
...
class RideProvider extends ChangeNotifier {
  RideState _state = RideState();

  RideState get state => _state;

  void updateState(RideState newState) {
    _state = newState;
    notifyListeners();
  }
...
class CustomerBookRideCleanPage extends StatefulWidget {
  const CustomerBookRideCleanPage({super.key});

  @override
  State<CustomerBookRideCleanPage> createState() => _CustomerBookRideCleanPageState();
}

class _CustomerBookRideCleanPageState extends State<CustomerBookRideCleanPage> {
  ...
  Widget _buildMap(RideState state) {
  ...
  Widget _buildNotificationBanner(RideState state) {
  ...
  Widget _buildLocationSelectors(RideState state) {
    return Material(
      elevation: 8,
  ...
  Widget _buildVehicleSelector(RideState state) {
    return Positioned(
      left: 0,
      right: 0,
      bottom: 210,
          ...
          itemBuilder: (context, index) {
            ...
            final fare = PricingService.calculateFare(
              ...
              distanceKm: (state.pickupLocation != null && state.destination != null)
                  ? _calculateDistance(state.pickupLocation!, state.destination!)
                  : 5.0,
              durationMin: 15,
            );
            return VehicleTile(
  ...
  Widget _buildBookingOverlay(RideState state) {
